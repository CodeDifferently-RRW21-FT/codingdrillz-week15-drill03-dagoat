...
public class Solution {
    ...
    public Integer[] numberFamily(Integer number, Integer[] possibleFamilyMembers){
        ...
        Map<Integer, Set<Integer>> families = new HashMap<>();
        Set<Integer> fam = new TreeSet<>();
        families.put(possibleFamilyMembers[0], fam);
        for(int j =0; j < possibleFamilyMembers.length; j++){
            for(int x =j+1; x < possibleFamilyMembers.length; x++){
                ...
                if((nextValue - currentValue) <=1){
                    fam.add(nextValue);
                    fam.add(currentValue);
                }else if((nextValue-currentValue) >1){
                    fam = new TreeSet<>();
                    families.put(currentValue, fam);
                }
            }
        }
        Integer[] response = null;
        for(Map.Entry<Integer, Set<Integer>> entry: families.entrySet()){
